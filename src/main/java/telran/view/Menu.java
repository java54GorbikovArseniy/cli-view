...
public class Menu implements Item{
    ...
    public void perform(InputOutput io) {
        ...
        do {
            displayItems(io);
            int itemNumber = io.readNumberRange("Enter item number", "Not existing item", 1,items.length).intValue();
            try {
    ...
    private void displayItems(InputOutput io) {
        IntStream.rangeClosed(1, items.length).forEach(i -> io.writeLine(String.format("%d.%s", i, items[i-1].displayName())));
    }
